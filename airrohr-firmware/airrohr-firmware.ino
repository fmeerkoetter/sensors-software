...
const uint8_t stop_SDS_cmd[] PROGMEM = {
	0xAA, 0xB4, 0x06, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x05, 0xAB
};
const uint8_t continuous_mode_SDS_cmd[] PROGMEM = {
	0xAA, 0xB4, 0x08, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x07, 0xAB
};
const uint8_t version_SDS_cmd[] PROGMEM = {
	0xAA, 0xB4, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x05, 0xAB
};
...
void HPMSensorDevice::init()
{
	if (hpm_read) {
		debug_out(F("Read HPM..."), DEBUG_MIN_INFO, 1);
		cmd(HPM_START);
		delay(100);
		cmd(HPM_CONTINUOUS_MODE);
		delay(100);
...
void HPMSensorDevice::cmd(const uint8_t cmd) {
...
String HPMSensorDevice::readSensor() {
	String s;
	char buffer;
	int value;
	int len = 0;
...
//			"aa" = 170, "ab" = 171, "c0" = 192
			...
			if (len == 32) {
				...
				if (checksum_ok == 1 && (long(act_milli - starttime) > (long(sending_intervall_ms) - long(READINGTIME_MS)))) {
					if ((! isnan(pm10_serial)) && (! isnan(pm25_serial))) {
						_pm10Sum += pm10_serial;
						_pm25Sum += pm25_serial;
						if (_pm10Min > pm10_serial) {
	...
	if (send_now) {
		...
		if (_valCount > 0) {
			last_value_HPM_P1 = double(_pm10Sum) / (_valCount * 1.0);
			last_value_HPM_P2 = double(_pm25Sum) / (_valCount * 1.0);
...
String SDS_version_date() {
	...
	sdsSensorDevice.cmd(SDS_VERSION_DATE);
...
#define strcpyFromJSON(key) if (json.containsKey(#key)) strcpy(key, json[#key]);
					...
					setFromJSON(sdsSensorDevice.sds_read);
...
#define copyToJSON_String(varname) json_string += Var2Json(#varname,String(varname));
	...
	copyToJSON_Bool(sdsSensorDevice.sds_read);
...
void webserver_config() {
	...
	if (server.method() == HTTP_GET) {
		...
		if (! wificonfig_loop) {
			...
			page_content += F("</b><br/>");
			page_content += form_checkbox("sds_read", FPSTR(INTL_SDS011), sdsSensorDevice.sds_read);
...
#define readPasswdParam(param) if (server.hasArg(#param)){ masked_pwd = ""; for (uint8_t i=0;i<server.arg(#param).length();i++) masked_pwd += "*"; if (masked_pwd != server.arg(#param) || server.arg(#param) == "") { server.arg(#param).toCharArray(param, sizeof(param)); } }
		...
		if (! wificonfig_loop) {
			...
			readBoolParam(sdsSensorDevice.sds_read);
...
#undef readPasswdParam
		...
		page_content += line_from_value(tmpl(FPSTR(INTL_LESE), "SDS"), String(sdsSensorDevice.sds_read));
...
void webserver_values() {
	if (WiFi.status() != WL_CONNECTED) {
		...
		if (sdsSensorDevice.sds_read) {
			page_content += empty_row;
			page_content += table_row_from_value(FPSTR(SENSORS_SDS011), "PM2.5", check_display_value(sdsSensorDevice.last_value_SDS_P2, -1, 1, 0), unit_PM);
			page_content += table_row_from_value(FPSTR(SENSORS_SDS011), "PM10", check_display_value(sdsSensorDevice.last_value_SDS_P1, -1, 1, 0), unit_PM);
		}
		...
		if (hpm_read) {
			page_content += empty_row;
			page_content += table_row_from_value(FPSTR(SENSORS_HPM), "PM2.5", check_display_value(hpmSensorDevice.last_value_HPM_P2, -1, 1, 0), unit_PM);
			page_content += table_row_from_value(FPSTR(SENSORS_HPM), "PM10", check_display_value(hpmSensorDevice.last_value_HPM_P1, -1, 1, 0), unit_PM);
		}
		if (dht_read) {
			page_content += empty_row;
...
void wifiConfig() {
	...
	debug_out(String(sdsSensorDevice.sds_read), DEBUG_MIN_INFO, 1);
...
void display_values() {
	...
	if (hpm_read) {
		pm10_value = hpmSensorDevice.last_value_HPM_P1;
		pm10_sensor = FPSTR(SENSORS_HPM);
		pm25_value = hpmSensorDevice.last_value_HPM_P2;
		pm25_sensor = FPSTR(SENSORS_HPM);
	}
	if (sdsSensorDevice.sds_read) {
		pm10_value = sdsSensorDevice.last_value_SDS_P1;
		pm10_sensor = FPSTR(SENSORS_SDS011);
		pm25_value = sdsSensorDevice.last_value_SDS_P2;
	...
	if (ppd_read || pms24_read || pms32_read || hpm_read || sdsSensorDevice.sds_read) {
...
void setup() {
	...
	uptime = starttime;
	sdsSensorDevice.starttime_SDS = millis();
	next_display_millis = millis() + 5000;
}
...
void loop() {
	...
	if (((act_milli - sdsSensorDevice.starttime_SDS) > sdsSensorDevice.SAMPLETIME_MS) ||
		((act_milli - starttime) > sending_intervall_ms)) {
		if (sdsSensorDevice.sds_read) {
			debug_out(F("Call sensorSDS"), DEBUG_MAX_INFO, 1);
			result_SDS = sdsSensorDevice.readSensor();
		...
		if (hpm_read) {
			...
			result_HPM = hpmSensorDevice.readSensor();
			sdsSensorDevice.starttime_SDS = act_milli;
		}
	}
